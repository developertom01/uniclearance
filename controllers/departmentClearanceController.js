...
const deleteClearance=async(req,res)=>{
    ...
    try {
        const clearance =await DepartmentClearance.findOne({where:{uuid:clearanceId}})
        if (!clearance) return res.status(404).json({message:"Department clearance does not exist"})
        await DepartmentClearance.destroy({where:{id:clearance.id}})
        res.json({status:"success",message:"You have succesfully deleted department clearance"})
    } catch (error) {
        res.status(500).json({message:"Unexpected error occued"})
        console.log(error);    }
}
