...
const index= async (req,res)=>{
    try {
        const departments = await DepartmentIssue.findAll({include:['student','department']})
...
const create=async(req,res)=>{
    ...
    try {
        ...
        if (!department) return res.status(403).json({message:"Department does not exist"})
        if (student.dataValues.id !==department.id) 
        return res.status(403).json({message:"Student does not belong to the specified department"})
        //Create issue
        ...
        //Get full json query
        issue = DepartmentIssue.findOne({where:{id:issue.id},include:['student','department']})
        res.status(201).json({
            status:"success",
            data:issue
        })
