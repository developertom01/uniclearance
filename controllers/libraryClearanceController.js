...
const create = async(req,res)=>{
    ...
    try {
       const student = await Student.findOne({where:{uuid:studentUid}})
        ...
        if(!student) return res.status(404).json({detail:"User does not exist"})
        let cleared =await LibraryClearance.create({studentId:studentId.id})
        cleared = await LibraryClearance.findOne({where:{id:cleared.id},include:"student"})
        res.status(401).json({
            status:"success",
...
const slefClear = async (req,res)=>{
    try {
        ...
        if(!student) return res.json({status:"error",detail:"Student information does not exist,Contact site admin"})
...
const remove = async (req,res)=>{
    try {
        ...
        const cleared = await LibraryClearance.findOne({where:{uuid:clearanceId}})
        if (!cleared) return res.status(404).json({"status":"errror",detail:"Library clearaane instance does not exist"})
